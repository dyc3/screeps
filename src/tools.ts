...
global.Market = {
    ...
    quickBuyEnergy(roomName: string) {
        if (!roomName) {
        ...
        if (!Game.rooms[roomName].terminal) {
            throw new Error(`Room ${roomName} does not have a terminal`);
        }

        const sellOrders = Game.market.getAllOrders(order => {
            if (order.type === ORDER_SELL && order.resourceType === RESOURCE_ENERGY && order.remainingAmount > 0) {
                // filter out sell orders that have low energy profit
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                const cost = Game.market.calcTransactionCost(100, roomName, order.roomName!);
        ...
        if (sellOrders.length === 0) {
            throw new Error(`No good energy sell orders found.`);
        }

        const order = sellOrders[0];
        const result = Game.market.deal(order.id, order.remainingAmount, roomName);

        return `Found ${sellOrders.length} energy sell orders. result: ${result}`;
    },
};
