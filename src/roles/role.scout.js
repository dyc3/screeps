...
function isRoomHostile(roomName) {
	let hasHostileOwner = false;
	if (Memory.roomInfo[roomName] && Memory.roomInfo[roomName].ownership) {
		hasHostileOwner =
			Memory.roomInfo[roomName].ownership != undefined
				? Memory.roomInfo[roomName].ownership.username != WHOAMI
				: false;
	}
...
const roleScout = {
	...
	findUnscoredRoom(creep) {
		...
		function findUnscoredAdjacentRecursive(roomName, excludeRoom = null, recursionLevel = 0) {
			...
			for (var e in exits) {
				var exit = exits[e]; //
				let hasHostileOwner = isRoomHostile(exit);
				let route = findRoute(creep.room, exit);
				if (
					(!Memory.roomInfo[exit] || (Memory.roomInfo[exit] && !isRoomScoreFresh(exit))) &&
					exit != creep.memory.homeRoom &&
					!hasHostileOwner &&
					route != -2
				) {
			...
			for (var e in exits) {
				...
				let possibleTarget = findUnscoredAdjacentRecursive(exit, roomName, recursionLevel + 1);
				if (
					possibleTarget != null &&
					(!Memory.roomInfo[possibleTarget] || !Memory.roomInfo[possibleTarget].score)
				) {
	...
	getRoomScore(room) {
		...
		if (!room.controller) {
			// FIXME: `room.controller` is supposed to return false when the room has no controller
			score -= 200;
		}
	...
	run(creep) {
		...
		if (!Memory.freshnessThreshold) {
			Memory.freshnessThreshold = defaultScoreFreshnessThreshold;
		}
		...
		if (creep.memory.targetRoomToScore) {
			if (creep.room.name != creep.memory.targetRoomToScore) {
				let route = Memory.tmpScoutRoute.slice(0); // .slice(0) is a hack to clone the array
				...
				console.log(
					...
					nextRoom,
					"target:",
					creep.memory.targetRoomToScore
				);
				...
				if (moveResult == ERR_NO_PATH) {
					console.log(
						creep.name,
						"WARN: No path to target room:",
						creep.memory.targetRoomToScore,
						", marking as fresh..."
					);
					Memory.roomInfo[creep.memory.targetRoomToScore] = {
						timestamp: Game.time,
						score: null,
						blocked: true,
					};
